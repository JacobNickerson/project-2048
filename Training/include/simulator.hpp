...
class Simulator {
    public:
        ...
        uint64_t convertBoardToPacked();
        std::array<uint8_t,16> convertBoardToUnpacked();
    ...
    private:
        ...
        // Helpers
        inline uint8_t shiftAmt(uint8_t index) { return 4*(3-index%4); }
        inline void setValue(uint8_t index, uint8_t val) { board[index/4] |= (val << shiftAmt(index)); } 
        bool rowCanMoveLeft(uint16_t row);
