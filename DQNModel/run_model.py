...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        default=False,
        help="Method of providing input, valid options: network, random, user",
    )
    ...
    match (input_method):
        ...
        case "network":
            if not args.network and not (args.q_network and args.target_network):
                print(
                    "error: --network or --q-network and --target-network required if not running with random input"
                )
                return
            if args.network and (args.q_network or args.target_network):
                print(
                    "error: --network not compatible with --q-network or --target-network"
                )
                return
        ...
        case _:
            print("error: invalid input method, valid options: network, random, user")
            return
