...
def generate_graphs(data_set_name, scores, max_tiles, board_states):
    ...
    tiles = np.array([16, 32, 64, 128, 256, 512, 1024, 2048, 4096])
...
def main():
    ...
    for step_count in step_counts:
        ...
        file_name = f"raw_data/{step_count}_{file_base}"
    ...
    plt.figure()
    plt.plot(step_counts, average_scores)
    ...
    plt.figure()
    plt.plot(step_counts, median_scores)
    plt.xlabel("Training Steps (million steps)")
    ...
    plt.figure()
    plt.plot(step_counts, high_scores)
    plt.xlabel("Training Steps (million steps)")
    ...
    tile_ticks = np.array([32, 64, 128, 256, 512, 1024, 2048, 4096])
    plt.figure()
    plt.plot(step_counts, median_max_tiles)
    plt.yscale("log", base=2)
    plt.yticks(tile_ticks, tile_ticks)
    plt.xlabel("Training Steps (million steps)")
    plt.ylabel("Median Max Tile")
    ...
    plt.figure()
    plt.plot(step_counts, highest_tiles)
    plt.yscale("log", base=2)
    plt.yticks(tile_ticks, tile_ticks)
    plt.xlabel("Training Steps (million steps)")
